...
CREATE OR REPLACE FUNCTION liste_livres_auteur_angela(p_nom_auteur TEXT) 
RETURNS VOID AS $$
DECLARE 
    rec_livres CURSOR FOR 
        SELECT l.titre FROM livre l
        JOIN auteur a ON l.id_auteur = a.id_auteur
        WHERE a.nom = p_nom_auteur;
    v_titres TEXT;
BEGIN
    OPEN rec_livres;
    LOOP
        FETCH rec_livres INTO v_titres;
        EXIT WHEN NOT FOUND;

        RAISE NOTICE 'Livre: %', v_titres;
    END LOOP;
    CLOSE rec_livres;
END;
$$ LANGUAGE plpgsql;

SELECT liste_livres_auteur_angela('HUGO');
